...
void CCheckBox::Draw(bool hover)
{
	...
	Render::gradient_verticle(a.x, a.y, 9, 9, Color(55, 55, 55, 255), Color(40, 40, 40, 245));
	Render::outlineyeti(a.x, a.y, 9, 9, Color(2, 2, 2, 255));

	if (Checked)
	{
		Render::gradient_verticle(a.x, a.y, 9, 9, Color(Options::Menu.ColorsTab.Menu.GetValue()), Color(12, 12, 12, 255));
		Render::outlineyeti(a.x, a.y, 9, 9, Color(2, 2, 2, 255));
	}
}
	...
	if (group_tabs.size())
	{
		...
		for (int i = 0; i < group_tabs.size(); i++)
		{
			...
			if (selected_tab == group_tabs[i].id)
			{
				...
				Render::Text2(text_position[0] - (text_size.right / 2), text_position[1], group_tabs[i].name.c_str(), Render::Fonts::MenuText, Color(Options::Menu.ColorsTab.Menu.GetValue()[0], Options::Menu.ColorsTab.Menu.GetValue()[1], Options::Menu.ColorsTab.Menu.GetValue()[2], MenuAlpha));
			}
...
void CSlider::Draw(bool hover)
{
	...
	Render::gradient_verticle(a.x, a.y, m_iWidth, 7, Color(62, 62, 62, 255), Color(55, 55, 55, 245));
	...
	Render::outlineyeti(a.x, a.y, m_iWidth, 7, Color(2, 2, 2, 255));
	...
	Render::text_yeti(a.x + Location - (txtSize.right / 2), a.y + 7 - (txtSize.bottom / 2), buffer, Render::Fonts::MenuBold, Color(180, 180, 180, 255));
}
...
void CKeyBind::Draw(bool hover)
{
	...
	{
		Render::text_yeti(a.x, a.y, "[ _ ]", Render::Fonts::MenuText, Color(110, 110, 110, 255));
	}
	else
	{
		if (key == -1)
		...
		else
		{
			...
			Render::text_yeti(a.x, a.y, NameOfKey, Render::Fonts::MenuText, Color(110, 110, 110, 255));
		}
	}
...
void CButton::Draw(bool hover)
{
	POINT a = GetAbsolutePos();
	if (hover)
		Render::gradient_verticle(a.x, a.y, m_iWidth, m_iHeight, Color(55, 55, 55, 255), Color(55, 55, 55, 255));
	else
		Render::gradient_verticle(a.x, a.y, m_iWidth, m_iHeight, Color(45, 45, 45, 255), Color(45, 45, 45, 255));

	Render::outlineyeti(a.x, a.y, m_iWidth, m_iHeight, Color(2, 2, 2, 255));
	...
	Render::text_yeti(TextX, TextY, Text.c_str(), Render::Fonts::MenuText, Color(180, 180, 180, 200));
}
...
void CComboBoxYeti::Draw(bool hover)
{
	...
	Render::gradient_verticle(a.x, a.y, m_iWidth, 20, Color(45, 45, 45, 255), Color(45, 45, 45, 255));
	if (GUI.IsMouseInRegion(Region)) Render::gradient_verticle(a.x, a.y, m_iWidth, 20, Color(55, 55, 55, 255), Color(55, 55, 55, 255));
	Render::outlineyeti(a.x, a.y, m_iWidth, 20, Color(2, 2, 2, 255));

	if (Items.size() > 0)
	{
		RECT txtSize = Render::GetTextSize(Render::Fonts::MenuText,GetItem().c_str());
		Render::text_yeti(a.x + 10, a.y + (Region.bottom / 2) - (txtSize.bottom / 2), GetItem().c_str(), Render::Fonts::MenuText, Color(180, 180, 180, 245));

		if (IsOpen)
		{
			Render::gradient_verticle(a.x, a.y + 20, m_iWidth, Items.size() * 20, Color(45, 45, 45, 255), Color(45, 45, 45, 255));
			...
			for (int i = 0; i < Items.size(); i++)
			{
				...
				{
					Render::gradient_verticle(a.x, a.y + 20 + i * 20, m_iWidth, 20, Color(35, 35, 35, 255), Color(35, 35, 35, 255));
				}

				RECT control_textsize = Render::GetTextSize(Render::Fonts::MenuText,Items[i].c_str());
				if (i == SelectedIndex)
					Render::text_yeti(a.x + 10, a.y + 20 + (i * 20) + 10 - (control_textsize.bottom / 2), Items[i].c_str(), Render::Fonts::MenuText, Color(Options::Menu.ColorsTab.Menu.GetValue()));
				else
					Render::text_yeti(a.x + 10, a.y + 20 + (i * 20) + 10 - (control_textsize.bottom / 2), Items[i].c_str(), Render::Fonts::MenuText, Color(180, 180, 180, 245));
			}
			Render::outlineyeti(a.x, a.y + 20, m_iWidth, Items.size() * 20, Color(2, 2, 2, 255));
		}
	}
	Vertex_t Verts2[3];
	...
	if (Items.size() > 0)
	{
		...
		for (int i = ScrollTop; (i < Items.size() && drawnItems < ItemsToDraw); i++)
		{
			...
			if (i == SelectedIndex)
			{
				...
				Render::gradient_verticle(ItemRegion.left, ItemRegion.top, ItemRegion.right, ItemRegion.bottom, Color(15,15, 15, 255), Color(Options::Menu.ColorsTab.Menu.GetValue()));
			}
