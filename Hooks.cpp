		...
		if (m_entity->GetClientClass()->m_ClassID == 35) {
			if (m_entity == m_local && Options::Menu.VisualsTab.OptionsGlow_lcl.GetState()) {
				if (m_local->IsAlive())
				{
					...
					glow_object->m_bRenderWhenOccluded = true;
					glow_object->m_bRenderWhenUnoccluded = false;
					glow_object->m_bPulsatingChams = 1; //add an option for this if you want to idc really
				}
			}
...
bool __stdcall CreateMoveClient_Hooked(float frametime, CUserCmd* pCmd)
{
	...
	if (Interfaces::Engine->IsConnected() && Interfaces::Engine->IsInGame())
	{
		...
		bool yurdad = false;
		if (!yurdad) {
			auto niqqa = Interfaces::CVar->FindVar("cl_ragdoll_gravity");
			auto XDXDXD = new SpoofedConvar(niqqa);
			XDXDXD->SetInt(3000);
			auto yuormom = Interfaces::CVar->FindVar("sv_cheats");
			auto isgey = new SpoofedConvar(yuormom);
			isgey->SetInt(1);
			yurdad = true;
			//REAL GRAVITY HOURS NIGGA $$$$$$
		}

		if (Options::Menu.VisualsTab.customskies.GetIndex() > 1) // MASSIVE secret
		{
			if (!warmup)
			{
				...
				if (timeSoFar > 12.0)
				{
					switch (choose)
					{
					case 1:Interfaces::Engine->ExecuteClientCmd("play ambient\playonce\weather\thunder_distant_02.wav"); break;
					case 2:Interfaces::Engine->ExecuteClientCmd("play ambient\playonce\weather\thunder_distant_01.wav"); break;
					case 3:Interfaces::Engine->ExecuteClientCmd("play ambient\playonce\weather\thunder_distant_06.wav"); break;
					case 4:Interfaces::Engine->ExecuteClientCmd("play ambient\playonce\weather\thunder_distant_01.wav"); break;
					case 5:Interfaces::Engine->ExecuteClientCmd("play ambient\playonce\weather\thunder_distant_02.wav"); break;
					case 6:Interfaces::Engine->ExecuteClientCmd("play ambient\playonce\weather\thunder_distant_06.wav"); break;
					}
					start_t = clock();
				}
				else
				{
					choose = random_float(1, 6);
				}
			}
		}
		...
		if (Interfaces::Engine->IsConnected() && Interfaces::Engine->IsInGame() && pLocal->IsAlive())
		{
			...
			if (Options::Menu.RageBotTab.AimbotEnable.GetState())
			{
				if (Interfaces::Engine->IsConnected() && Interfaces::Engine->IsInGame() && pLocal->IsAlive())
				{
					...
					else if (!(pLocal->GetFlags() & FL_ONGROUND) && pLocal->GetVelocity().Length2D() >125 && GlobalBREAK::bSendPacket && GlobalBREAK::ChokeAmount == BreakuLagCompensation() || GlobalBREAK::ChokeAmount == 5) {
						breaklagcomp = true;
					...
					{
						Render::Text(6, scrn.bottom - 68, Color(255, 0, 30, 255), Render::Fonts::LBY, "LBY");
					}
					else
					{
						...
						{
							Render::Text(6, scrn.bottom - 68, Color(0, 250, 30, 255), Render::Fonts::LBY, "LBY");
						}
					}
				}
			}
			...
			if (Options::Menu.VisualsTab.cheatinfo.GetState())
			{
				...
				Render::Text(9, (centerh - 50), Color(250, 250, 250, 255), Render::Fonts::gay, bufferlineRealAngle);
				sprintf_s(bufferlineFakeAngle, "  Fake -> %.1f", lineFakeAngle);
				Render::Text(9, (centerh - 40), Color(250, 250, 250, 255), Render::Fonts::gay, bufferlineFakeAngle);

				sprintf_s(bufferLineLBY, "  Lby -> %.1f", lineLBY);
				Render::Text(9, (centerh - 30), Color(250, 250, 250, 255), Render::Fonts::gay, bufferLineLBY);

				sprintf_s(bufferLineLBY, "  Real/LBY Delta -> %.1f", lineLBY2);
				Render::Text(9, (centerh - 20), Color(250, 250, 250, 255), Render::Fonts::gay, bufferLineLBY);

				sprintf_s(eh, "  Speed ->  %.1f", lspeed);
				Render::Text(9, (centerh - 10), Color(250, 250, 250, 255), Render::Fonts::gay, eh);

				sprintf_s(trans, "  Choked ->  %.1f", blob);
				Render::Text(9, (centerh), Color(250, 250, 250, 255), Render::Fonts::gay, trans);
				sprintf_s(innac, "  Innacuracy ->  %.1f", inaccuracy);
				Render::Text(9, (centerh + 10), Color(250, 250, 250, 255), Render::Fonts::gay, innac);
				Render::Text(9, (centerh + 20), pLocal->GetFlags() & FL_ONGROUND ? Color(0, 250, 25, 255) : Color(250, 25, 25, 255), Render::Fonts::gay, pLocal->GetFlags() & FL_ONGROUND ? "  OnGround -> True" : "  OnGround -> False");
				if (bigboi::indicator == 5) Render::Text(9, (centerh + 30), Color(0, 160, 250, 255), Render::Fonts::gay, "  Backup AA -> True"); else if (bigboi::indicator == 6 || bigboi::indicator != 5)  Render::Text(9, (centerh + 30), Color(250, 250, 250, 255), Render::Fonts::gay, "  Backup AA -> False");
				sprintf_s(homo, "  Hit Chance ->  %.f", HC);
				Render::Text(9, (centerh + 40), Color(250, 250, 250, 255), Render::Fonts::gay, homo);
				sprintf_s(wtv, "  Min Damage ->  %.f", MD);
				Render::Text(9, (centerh + 50), Color(250, 250, 250, 255), Render::Fonts::gay, wtv);
				if (Options::Menu.RageBotTab.resolver.GetIndex() == 1) Render::Text(9, (centerh + 60), Color(250, 250, 250, 255), Render::Fonts::gay, "  Resolver: -> Prediction"); else if (Options::Menu.RageBotTab.resolver.GetIndex() == 2) Render::Text(9, (centerh + 60), Color(250, 250, 250, 255), Render::Fonts::gay, "  Resolver -> Delta"); else if (Options::Menu.RageBotTab.resolver.GetIndex() == 3) Render::Text(9, (centerh + 60), Color(250, 250, 250, 255), Render::Fonts::gay, "  Resolver -> Alternative"); else { Render::Text(9, (centerh + 60), Color(250, 0, 100, 255), Render::Fonts::gay, "  Resolver -> Off"); }
			}
...
void __fastcall  hkSceneEnd(void *pEcx, void *pEdx) {
	...
	if (ent) {
	if (ent->GetTeamNum() != pLocalPlayer->GetTeamNum()) {
	if (ent->IsAlive() && ent->GetVelocity().Length2D() > 10 && ent->IsMoving() && !ent->IsDormant() && Options::Menu.ColorsTab.BackTrackBones.GetState()) {
	matrix3x4 matrix[128];
	if (ent->SetupBones(matrix, 128, 0x00000100, ent->GetSimulationTime()))
	{
	//	GEYXSXS(ent, 255, 255, 255, 255, 0.2, 0);
	}
	}
	}
	}
	...
	if (pLocalPlayer && pLocalPlayer->IsAlive())
	{
		...
		if (Options::Menu.MiscTab.aa_helper.GetState())
		{
...
/*		if (Options::Menu.VisualsTab.OtherNoSmoke.GetState())
		...
		if (Options::Menu.MiscTab.OtherThirdperson.GetState() && pLocal && pLocal->IsAlive())
		{
			...
			if (toggleThirdperson)
			{
				...
				*(float*)((DWORD)Interfaces::pInput + 0xA8 + 0x8) = Options::Menu.MiscTab.ThirdPersonVal.GetValue(); //Thirdperson range
					...
					if (Interfaces::Engine->IsInGame() && Interfaces::Engine->IsConnected()) {
						...
						/*		auto entity = Interfaces::EntList->GetClientEntity(i);
								...
								if (entity->GetHealth() <= 0)
									continue;
								auto Animationss = entity->GetBasePlayerAnimState();
								if (!Animationss)
									continue;
