...
void CMiscHacks::FakeWalk0(CUserCmd* pCmd, bool &bSendPacket)
{
	...
	if (key1 >0 && GetAsyncKeyState(key1) && !Options::Menu.m_bIsOpen)
	{
		...
		/*if (fabs(GlobalBREAK::NextPredictedLBYUpdate - Interfaces::Globals->curtime) <= 0.1)
		{
			pCmd->forwardmove = 450;
			rotate_movement(CalcAngleFakewalk(Vector(0, 0, 0), pLocal->GetVelocity()).y + 180.f, pCmd);
		}*/
		float nani = Options::Menu.MiscTab.FakeWalkSpeed.GetValue() / 14;
	//	pLocal->client_side_animation() = true;
	...
	//	pLocal->client_side_animation() = false;
	}
}
