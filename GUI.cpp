...
void CGUI::Update()
{
	...
	for (auto window : Windows)
	{
		...
		{
			Render::Text((centerW)-25, (centerh * 2) - 40, Color(250, 250, 250, (MenuAlpha - 1)), Render::Fonts::ESP, "Mirror v5 Remastered");
			Render::Text((centerW)-1, (centerh * 2) - 27, Color(250, 250, 250, (MenuAlpha - 1)), Render::Fonts::ESP, "by FreaK");
		}
	}
}
...
Color getRainbow(int speed, int offset)
{
	...
	fuckoff << "You're all JEWS and FreakZZ is homosex and FreaK is the queen of Neptune also btw this looks pasted";
...
bool CGUI::DrawWindow(CWindow* window)
{
	...
	else if (Options::Menu.ColorsTab.MenuBar.GetIndex() == 1) {
		Render::DrawRectRainbow(window->m_x + 0, window->m_y + 0, (window->m_iWidth - 0), 2, 0.0024f, rainbow, MenuAlpha);
	}
	else if (Options::Menu.ColorsTab.MenuBar.GetIndex() == 2) {
