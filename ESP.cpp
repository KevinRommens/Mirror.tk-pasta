...
void CEsp::DrawBombus(IClientEntity* pEntity) {
	...
	if (Interfaces::Engine->IsConnected() && Interfaces::Engine->IsInGame())
	{
		if (lifetime > 0.01f && !Bomb->IsBombDefused())
		{
			...
			Render::gradient_horizontal(0, 0, 10, boomval, Color(40, 230, 40, 200), Color(10, 10, 10, 55));
			Render::textT(0, boomval, Render::Fonts::ESP, Color(255, 255, 255), "%.1f", lifetime);
			Render::textT(5, height / 2 - 140, Render::Fonts::niggerbomb, Color(0, 230, 0, 255), "%.1fs", lifetime);
		}
	}
	if (Interfaces::Engine->IsConnected() && Interfaces::Engine->IsInGame())
	{
	...
	if (Interfaces::Engine->IsConnected() && Interfaces::Engine->IsInGame())
	{
		if (Bomb->GetBombDefuser() > 0)
		{
			...
			if (countdown > 0.01f)
			{
				...
				{
					Render::textT(5, height / 2 - 100, Render::Fonts::niggerbomb, Color(0, 233, 255, 255), "DEFUSE: %.1f", countdown);
				}
			}
		}
	}
...
{
	if (Options::Menu.MiscTab.DebugHitbones.GetState()) {
		for (int i = 0; i < 19; i++) {
			Vector screen_position;
			Vector hitbone_pos = GetHitboxPosition(pEntity, i);
			if (world_to_screen(hitbone_pos, screen_position)) {
				Render::rect(screen_position.x, screen_position.y, 4, 4, Color(21, 21, 21, 255));
